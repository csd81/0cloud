...
Ebben a fejezetben a technológiai fejlődés áttekintése nem annyira az egyes technológiák műszaki részleteire összpontosít (feltételezzük, hogy az olvasók már ismerik ezeket az alapvető témákat), hanem inkább azokra a problémákra, amelyeket megpróbáltak megoldani, arra, hogy ezt milyen módon tették, és arra a fogalmi útra („a pontok összekötésére”), amely a felhőalapú számítástechnika megjelenéséhez vezetett. Az egyes alfejezetek végén kiemeljük, hogy az adott technológia vagy annak alapfogalmai hogyan illeszkednek a modern felhőrendszerek világába.
...
A számítógépekre építő modern vállalatok informatikai infrastruktúrájának működtetésében két, egymással ellentétes cél jelenik meg. Az egyik cél az infrastruktúra méretének minimalizálása, hogy a beruházási és üzemeltetési költségek alacsonyak maradjanak. A másik cél ugyanakkor az, hogy elegendő erőforrás álljon rendelkezésre a nyereséges működéshez szükséges feladatok ellátására. Ha egy vállalkozás számítási igénye állandó és kiegyensúlyozott, vagyis folyamatosan hasonló számú alkalmazást futtat, akkor a szükséges kapacitás viszonylag könnyen megtervezhető. Ingadozó terhelés esetén azonban a helyzet jóval összetettebb. A legrosszabb eset a kiszámíthatatlan, hirtelen megugró, lökésszerű terhelési mintázat.
...
|---|---:|---:|---|---:|---|
| `m4.large` | 2 | 8 | Csak EBS | 450 | Közepes |
| `m4.xlarge` | 4 | 16 | Csak EBS | 750 | Magas |
| `m4.2xlarge` | 8 | 32 | Csak EBS | 1 000 | Magas |
| `m4.4xlarge` | 16 | 64 | Csak EBS | 2 000 | Magas |
...
// Várakozás a művelet befejeződésére
try {
...
// A GCP-projekt azonosítója
// String projectId = "your-project-id";
// A GCS-bucketnek adott egyedi név
// String bucketName = "your-unique-bucket-name";
Storage storage = StorageOptions.newBuilder().setProjectId(projectId)
.build().getService();
// A további érvényes tárolási osztályokért lásd a StorageClass dokumentációját:
// https://googleapis.dev/java/google-cloudclients/latest/com/google/cloud/storage/StorageClass.html
StorageClass storageClass = StorageClass.COLDLINE;
// A további érvényes elhelyezkedésekért lásd:
...
Ezt a különbséget jól megvilágítja egy egyszerű statisztikai számítási feladat. Tegyük fel, hogy $N$ darab $x_i,\ i = 1, \ldots, N$ adatminta varianciáját kell kiszámítanunk. A variancia a jól ismert képlettel definiált:

$$Var(X) = \frac{1}{n}\sum_{i=1}^{n} (x_i - \mu)^2$$

ahol $\mu$ az $x_i$ minták számtani átlaga:

$$\mu = \frac{1}{n}\sum_{i=1}^{n} x_i$$

Ha hozzáférünk a teljes adathalmazhoz, vagyis az összes $N$ elemhez a memóriában vagy egy bemeneti fájlban, az egyik megoldás egy kétmenetes (two-pass) algoritmus használata. Az első menetben az átlagot számítjuk ki, a másodikban pedig a varianciát. A Java megvalósítás a következő:
...
Csábító lehet a két menet kiváltása úgy, hogy az implementációt egyetlen ciklusba vonjuk össze az alábbi, algebrai átalakítással kapott képlet segítségével:

$$Var(X) = \frac{1}{n}\sum_{i=1}^{n}(x_i - \mu)^2 = \frac{1}{n}\sum_{i=1}^{n} x_i^2 - \mu^2 = \frac{1}{n}\left[\sum_{i=1}^{n} x_i^2 - \frac{1}{n}\left(\sum_{i=1}^{n} x_i\right)^2\right]$$

Az első tag a négyzetek összege, a második pedig az elemek összegének négyzete osztva az elemszámmal. Ennek Java megvalósítása a következő:
...
Ennek a megoldásnak az a problémája, hogy a `sumOfSquares` és a `(sum * sum)/data.length` értékei nagyon hasonlóak lehetnek, így a különbségük olyan kicsi értéket eredményezhet, amelyet a lebegőpontos ábrázolás rendelkezésre álló pontossága már nem tud megfelelően kezelni. Ezt a problémát katasztrofális kioltásnak (catastrophic cancellation) nevezzük [16], és hibás eredményekhez vezet. Ennek megfelelően ezt az egymenetes (one-pass) változatot a gyakorlatban nem szabad használni.

Olyan egymenetes (one-pass) algoritmust, amely helyes eredményt ad és nem szenved a katasztrofális kioltás problémájától, Welford mutatott be [17]. Ez egy egymenetes, más néven online algoritmus, amely az új minták beérkezésekor inkrementálisan frissíti az átlagot és a varianciát. Legyenek $m_k,\ k = 1, \ldots, N$ a részátlagok: $m_1 = x_1/1$, $m_2 = (x_1 + x_2)/2$, és így tovább. Belátható, hogy $m_k$ kifejezhető $m_{k-1}$ segítségével:

$$m_k = m_{k-1} + \frac{x_k - m_{k-1}}{k}$$

A variancia számlálója ($s_n$) hasonlóan frissíthető:

$$s_k = s_{k-1} + (x_k - m_{k-1})(x_k - m_k)$$

majd $Var(X) = s_n/n$. Az algoritmus jelentősége abban áll, hogy numerikusan stabil egymenetes módszert ad az átlag és a variancia kiszámítására, és úgy is használható, hogy a teljes adathalmaz nem ismert előre. Ismeretlen hosszúságú adatsorokra is alkalmazható. Egyben azt is jól szemlélteti, hogy a stream-algoritmusok gyakran algoritmikai változtatásokat követelnek meg, és sokszor vissza kell nyúlni az alapvető matematikai levezetéshez ahhoz, hogy az adatfolyam-feldolgozásban is használható megoldást kapjunk.
...
Bármely stream rendszer egyik legfontosabb tulajdonsága a megbízhatóság és a hibatűrés. Ezt rendszerint az üzenetkézbesítési garanciák fejezik ki. Három alapvető üzenetkézbesítési szemantika létezik elosztott rendszerekben: *legfeljebb egyszer* (`At-Most-Once`), *legalább egyszer* (`At-Least-Once`) és *pontosan egyszer* (`Exactly Once`). A *legfeljebb egyszer* szemantika azt jelenti, hogy minden üzenet kézbesítését legfeljebb egyszer kísérli meg a rendszer. Ha a kézbesítés valamilyen okból meghiúsul, nem történik újraküldés. Röviden: az ilyen rendszerben üzenetek elveszhetnek. A *legalább egyszer* szemantika garantálja, hogy minden üzenet eljut a fogyasztóhoz, de bizonyos üzenetek hiba esetén többször is megérkezhetnek az ismételt küldések miatt. Ilyen rendszerekben az alkalmazásnak nyomon kell követnie az üzeneteket, és kezelnie kell a felesleges duplikációkat. A *pontosan egyszer* a legerősebb garancia: minden üzenet pontosan egyszer jut el a fogyasztóhoz. Az alkalmazásnak nem kell a duplikált üzenetek miatt aggódnia. Az üzenetkézbesítési szemantikák rendszerint együtt járnak az üzenetsorrend garanciáival is. A *legfeljebb egyszer* és a *legalább egyszer* esetén az üzenetek kézbesítési sorrendje nem feltétlenül egyezik a küldési sorrenddel. A *pontosan egyszer* szemantika pontos üzenetsorrendet is biztosít.
...
A Cloud Functions a felhőalapú végrehajtási technológia legújabb fejleményeit képviseli. A függvények fő jellemzője, hogy lehetővé teszik a felhasználók számára, hogy közvetlenül a felhőbe töltsenek fel kódot anélkül, hogy külön futtatási infrastruktúrát kellene létrehozniuk. Ennek megfelelően a függvényeket szerver nélküli kódvégrehajtásnak tekintjük, és a felhőszolgáltatók ma már új szolgáltatástípusként, Függvény mint Szolgáltatásként (Function-as-a-Service – FaaS) hivatkoznak rájuk.
...
A GCP a függvényekre a következő végrehajtási garanciákat adja: a HTTP-függvények *legfeljebb egyszer* (`At-Most-Once`), míg a háttérfüggvények *legalább egyszer* (`At-Least-Once`) szemantikával hívódnak meg.

A Cloud Functions futását eseményindítók (triggerek) indítják el. A triggerek típusai a következők lehetnek: HTTP triggerek, Cloud Pub/Sub triggerek, Cloud Storage triggerek, közvetlen (Direct) triggerek, Cloud Firestore triggerek, Analytics for Firebase triggerek, Firebase Realtime Database triggerek és Firebase Authentication triggerek. A következő fejezetben a Cloud Pub/Sub mechanizmust vizsgáljuk meg részletesebben, hogy megértsük az általános esemény- és üzenetkézbesítési mechanizmusokat.
...
1. Egy közzétevő alkalmazás létrehoz egy témakört a Pub/Sub szolgáltatásban, majd üzeneteket küld erre a témakörre. Az üzenet tartalmaz egy hasznos tehert (payload) és opcionális attribútumokat, amelyek leírják a hasznos teher tartalmát.
...
4. A feliratkozó kétféleképpen kaphat üzenetet: vagy a Pub/Sub leküldéses (Push) módon továbbítja azt a feliratkozó által megadott végpontra, vagy a feliratkozó lekéréses (Pull) módon maga kéri le a szolgáltatástól.
...
A Pub/Sub alapértelmezés szerint *legalább egyszer* (`At-Least-Once`) szemantikával kézbesíti az üzeneteket. Az üzenetkézbesítés egyetlen kivétele az, ha egy üzenet a létrejöttétől számított hét napon belül nem kézbesíthető; ebben az esetben hét nap után kézbesítés nélkül törlődik. Hasonlóképpen, ha egy üzenet létrejöttekor nincs hozzá tartozó feliratkozás, az az üzenet egyáltalán nem kerül kézbesítésre.
...
public static void main(String... args) throws Exception {
// TODO: a minta futtatása előtt cseréld le ezeket a változókat.
String projectId = "your-project-id";
...
try {
// Közzétevő létrehozása az alapértelmezett beállításokkal, a témakörhöz kötve
publisher = Publisher.newBuilder(topicName).build();
List<String> messages = Arrays.asList("first message", "second message");
for (final String message : messages) {
ByteString data = ByteString.copyFromUtf8(message);
PubsubMessage pubsubMessage = PubsubMessage.newBuilder().setData(data).build();
// Közzététel után a szerver által generált üzenetazonosítót adja vissza (a témakörön belül egyedi)
ApiFuture<String> future = publisher.publish(pubsubMessage);
// Aszinkron visszahívás hozzáadása a siker/hiba kezeléséhez
...
// Közzététel után a szerver által generált üzenetazonosítót adja vissza
// (a témakörön belül egyedi)
...
if (publisher != null) {
// Ha végeztünk a közzétevővel, állítsuk le az erőforrások felszabadításához.
publisher.shutdown();
...
A második példa azt szemlélteti, hogyan lehet üzenetet fogadni egy alkalmazásban. A Pub/Sub-ban kétféle üzenetkézbesítés létezik: lekéréses (`Pull`) és leküldéses (`Push`). Lekéréses kézbesítés esetén a feliratkozónak explicit módon le kell kérnie az üzenetet a Pub/Sub rendszertől. Leküldéses módban a Pub/Sub rendszer közvetlenül elküldi az üzenetet a feliratkozónak anélkül, hogy külön kérést várna.
...
// Aszinkron üzenetfogadó példányosítása.
...
// A feliratkozó indítása.
subscriber.startAsync().awaitRunning();
System.out.printf("Listening for messages on %s:\n", subscriptionName.toString());
// A feliratkozó 30 másodpercig fut, hacsak nem lép fel helyrehozhatatlan hiba.
subscriber.awaitTerminated(30, TimeUnit.SECONDS);
} catch (TimeoutException timeoutException) {
// 30 másodperc után a feliratkozó leállítása; az üzenetek fogadása véget ér.
...
Ez a minta jól szemléltette a felhőszolgáltatás-orkesztráció kulcsfogalmait, valamint azt az egyszerűséget és eleganciát, amellyel a szolgáltatások közötti laza csatolású üzenet- és eseménykommunikáció használható. A fejlesztőnek nem kell az infrastruktúra-kezelés részleteivel foglalkoznia, kizárólag az alkalmazáslogikára koncentrálhat. A Cloud Functions automatikus skálázási tulajdonságának köszönhetően az alkalmazás tetszőleges számú klienst képes kiszolgálni további beavatkozás nélkül.

Az érdeklődő olvasó további szolgáltatás-orkesztrációs mintákért forduljon a Google Cloud dokumentációhoz.

## 13. Következtetések

Ennek a könyvnek az volt a célja, hogy háttéranyagot biztosítson a Pannon Egyetem felhőprogramozás MSc kurzusához. Ennek megfelelően nem az volt a szándéka, hogy a felhőalapú számítástechnika minden elméleti és gyakorlati aspektusát lefedő, lezárt mű legyen. A kurzus fókusza a felhőrendszerek programozásán van, szemben a felhő-infrastruktúrák létrehozásával és menedzselésével, ezért a rendszerüzemeltetési témákat nagyrészt figyelmen kívül hagytuk. A cél az sem volt, hogy olyan részletességgel tárgyaljuk a programozást, amilyenre professzionális felhőfejlesztőknek szükségük lenne. Ugyanakkor bízom benne, hogy a könyv hasznos bevezetést nyújtott a legfontosabb alapfogalmakba. Reményeim szerint az itt bemutatott gyakorlati programozási részletek elegendő alapot adnak az érdeklődő hallgatóknak ahhoz, hogy önállóan is elinduljanak a felhőalapú számítástechnika rendkívül izgalmas világában.
